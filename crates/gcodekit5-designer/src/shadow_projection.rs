...
impl ShadowProjector {
    ...
    fn project_perspective(&self, mesh: &Mesh3D, distance: f32) -> Result<Vec<Shape>> {
        debug!(
            "Performing perspective projection with distance {}",
            distance
        );
    ...
    fn create_view_transform(&self) -> Matrix4<f32> {
        ...
        Matrix4::look_at_rh(&Point3::new(0.0, 0.0, 0.0), &Point3::from(forward), &up)
    }
    ...
    fn calculate_adaptive_layer_height(
        &self,
        mesh: &Mesh3D,
        current_z: f32,
    ...
    pub fn multi_angle_projection(&self, mesh: &Mesh3D, angles: &[f32]) -> Result<Vec<Vec<Shape>>> {
        ...
        for &angle in angles {
            info!(
                "Generating projection at angle {} degrees",
                angle.to_degrees()
            );
